...
- The site has outgrown the Scotland trip: `#manchester` is a matchday guide (Britannia Hotel on Portland Street, Manchester City vs Sunderland at the Etihad) rendered from `TRIP.manchester` by `manchesterHTML()`, mirroring how `TRIP.edinburgh` works. Content changes go in the data object, not the markup.
- The desktop nav row cannot hold unlimited links: `.nav-links` is `min-width:0` with hidden-scrollbar horizontal scroll so extra links never push the page wider, and the burger takes over below 1200px. Adding a nav link is safe; removing that CSS is not.
- Photo tool (`#photo` section, the IIFE at the end of the script): strips metadata from an uploaded image and hands back a clean PNG or JPG. Runs entirely client-side, no uploads. Same format in and out is a pure copy with the metadata cut out, no re-encode. **Both scanners are whitelists, deliberately**: a blacklist missed unknown chunk types and anything trailing the image. JPEG keeps SOI/EOI/SOS+scan/DQT/DHT/DAC/DRI/SOFn and APP14 (Adobe's colour transform), and stops at EOI so trailers — MPF second images, motion-photo MP4s, appended XMP — are cut. PNG keeps only IHDR/PLTE/IDAT/IEND/tRNS/gAMA/cHRM/sRGB/sBIT and the APNG chunks, and cuts anything past IEND. Bit depth, palette and animation survive PNG to PNG.
- `inventory()` verifies the output and must stay independent of the strippers. It enumerates what the finished file actually contains and flags anything outside the whitelist plus any trailing bytes. The earlier version re-ran the stripper and asked what it had removed, which only ever confirmed the stripper's own blind spots — it reported "0 metadata blocks left" on files that still carried data.
- The photo tool also reframes for Instagram: `IG` holds 1:1 (1080x1080), 4:5 (1080x1350), 9:16 (1080x1920) and 1.91:1 (1080x566). `outSize()` never upscales — if the source cannot fill the frame at 1080 the output is scaled down to whatever it can fill, and the panel says so. `compose()` does fill (centre crop) or fit (padded with a blurred, slightly dimmed copy of the image, or flat white/black). Choosing a frame always forces the canvas path, so `canCopy()` returns false and the lossless promise no longer applies — the UI says this outright.
