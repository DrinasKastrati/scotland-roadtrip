...
- Photo tool (`#photo` section, the IIFE at the end of the script): strips metadata from an uploaded image and hands back a clean PNG or JPG. Runs entirely client-side, no uploads. PNG goes through a canvas, which is pixel-exact and cannot carry metadata. JPG out of a JPEG source copies the entropy-coded scan through untouched and only drops the APPn/COM segments, so there is no re-encode — except when EXIF orientation is not 1, where the rotation has to be baked in. Canvas-encoded JPEGs are run back through the same stripper, because the browser's own encoder writes a JFIF header and an sRGB ICC profile of its own. APP14 (Adobe) is deliberately kept: it carries the colour transform, not personal data.
